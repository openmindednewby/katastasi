 ...
 */
import { changedFields, classify, SyncAction } from './classify.js';
...
export type PlanAction =
  ...
  | 'conflict' // both changed differently → flag, apply nothing
...
export function planSync(
  ...
  for (const l of locals) {
    ...
    const action = FROM_CLASSIFY[classify(st.base, l.record, remote.fields)];
    const item: PlanItem = {
      action, key: l.key, path: l.path, remoteId: remote.id, remoteRev: remote.rev, remoteUrl: remote.url,
      local: l.record, remote: remote.fields, base: st.base,
    };
    if (action === 'conflict') item.conflictFields = changedFields(l.record, remote.fields);
    items.push(item);
  }
...
export function planSummary(plan: Plan): Record<PlanAction, number> {
  const out = { skip: 0, push: 0, pull: 0, 'create-remote': 0, 'pull-create': 0, converged: 0, conflict: 0, 'local-deleted': 0, 'remote-deleted': 0 } as Record<PlanAction, number>;
